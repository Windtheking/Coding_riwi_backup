...
def Yes_No(x):
    ...
    while y > 0:
        ...
        if x == "y" or x == "n":
            ...
            y = 1
            print("\033[33m| Invalid values detected, please insert a valid selection \033[0m")
            x = error_targeting(str, "\033[32m| --> \033[0m")
...
def add_book_to_database():
    ...
    if answer1 == "y":
        ...
        try:
            if new_category in eternal_knowledge_library:
                formated_text(f"New category {new_category} created successfully")
        except:
            formated_text("An Error has ocurred with the new category creation, please try again")
    elif answer1 == "n":
...
#        formated_text("Please note, if you insert an unexisting subcategory you'll be notified")
        ...
        if checking != "Inesxistente":
            ...
            if checking2 != "None existent":
                formated_text("You inserted an existing book, reseting to menu")
                cls()
                browse_inventory()
            elif checking2 == "None existent":
                eternal_knowledge_library[f"{existing_category}"][f"{subcategory}"] = [sotck_count, price, author]
        formated_text(f"New book {subcategory} created successfully")
        print((rayita + igual*180 + rayita))
        time.sleep(2)
        cls()
...
def update_stock_books():
    ...
    match option_selected:
        case 1:
            model1("price", 1)
            formated_text("Value updated succesfully")
            time.sleep(2)
            browse_inventory()
        case 2:
            model1("stock", 0)
